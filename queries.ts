...
async function createUserTable() {
  ...
  `;
  await connection().query(query);  // Use pool para executar a query
}

async function addUser(user: User) {
  const query = 'INSERT INTO users (name, user, password, email) VALUES (?, ?, ?, ?)';
  const [result] = await connection().execute(query, [user.name, user.user, user.password, user.email]);
  return result;
}
...
async function getUserByUser(user: string, codigo_escola: string) {
  ...
  const [rows] = await connection().execute(query, [user, codigo_escola]);
  return rows;
}
...
async function getAlunoByCurso(id: string, codigo_escola: string) {
  ...
  const [rows] = await connection().execute(query, [id, codigo_escola]);
  return rows;
}
...
async function getAlunoBycurso(id_aluno: string, codigo_escola: string, id_curso: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno, codigo_escola, id_curso]);
  return rows;
}
...
async function getPacoteByAluno(id: string, codigo_escola: string) {
  ...
  const [rows] = await connection().execute(query, [codigo_escola, codigo_escola, id]);
  return rows;
}
...
async function getParcelas(id: string, codigo_escola: string, id_aluno: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno, codigo_escola, id]);
  return rows;
}
...
async function getNotas(id_modulo: string, codigo_escola: string, id_aluno_curso: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno_curso, id_aluno_curso, id_modulo, codigo_escola]);
  return rows;
}
...
async function getFrequenciaAula(codigo_escola: string, id_aluno_aluno: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno_aluno, codigo_escola]);
  return rows;
}
...
async function getFrequenciaPresenca(type: string, codigo_escola: string, id_aluno: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno, codigo_escola, type]);
  return rows;
}
...
async function getFrequenciaReposicao(codigo_escola: string, id_aluno: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno, codigo_escola]);
  return rows;
}
...
async function getModulo(codigo_escola: string, id_aluno_curso: string) {
  ...
  const [rows] = await connection().execute(query, [id_aluno_curso, codigo_escola]);
  return rows;
}

async function getMaterial(codigo_escola: string, id_modulo: string) {
  const query = 'select m.id_material, m.titulo, m.arquivo, m.data, md.descricao, m.tipo from modulo_material m left join modulos md on(md.id_modulo = m.id_modulo and md.codigo_escola = ?) where m.id_modulo = ? and m.codigo_escola = ? order by m.titulo asc;';
  const [rows] = await connection().execute(query, [codigo_escola ,id_modulo, codigo_escola]);
  return rows;
}
...
async function getMaterialById(codigo_escola: string, id_modulo: string, id_material: string) {
  ...
  const [rows] = await connection().execute(query, [codigo_escola ,id_modulo, codigo_escola, id_material]);
  return rows;
}
...
async function getCategorias(codigo_escola: string) {
  const query = 'SELECT pacotes.id_pacote, pacotes.codigo_escola, pacotes.nome, pacotes.contrato, pacotes.situacao, pacotes.id_contrato, pacotes.cadint, categoria_pacotes.id_categoria, categoria_pacotes.categoria, categoria_pacotes.icone, categoria_pacotes.descricao FROM pacotes LEFT JOIN categoria_pacotes ON pacotes.id_categoria = categoria_pacotes.id_categoria WHERE pacotes.situacao="A" and pacotes.cadint=1 and pacotes.codigo_escola= ? ORDER BY categoria_pacotes.id_categoria ';
  const [rows] = await connection().execute(query, [codigo_escola]);
  return rows;
}
